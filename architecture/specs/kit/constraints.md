...
version: 1.0
purpose: Define the format, generation algorithm, validation semantics, and usage of generated constraints.toml files
drivers:
...
- [Constraints Specification (constraints.toml)](#constraints-specification-constraintstoml)
  - [Overview](#overview)
  - [Location](#location)
...
`constraints.toml` is a **generated** kit-wide output file produced by the Blueprint Processor from `@cpt:heading` and `@cpt:id` markers across all artifact blueprints. It defines structural validation rules:
...
**Key properties**:
- Generated from all artifact blueprints in a kit — do not edit manually
- Kit-wide: one file per kit, aggregated from all artifact blueprints
...
| Marker | Role |
|--------|------|
| `@cpt:heading` | Heading constraint definition → `[[artifacts.<KIND>.headings]]` |
| `@cpt:id` | ID kind definition with cross-artifact reference rules → `[artifacts.<KIND>.identifiers]` |

Constraints are generated only from blueprints that have an `artifact` key in `@cpt:blueprint`. Codebase blueprints (without `artifact` key) do not contribute to constraints.toml.
...
1. For each installed blueprint with an `artifact` key:
   a. Collect all `@cpt:heading` TOML blocks → `[[artifacts.<KIND>.headings]]` entries.
   b. Collect all `@cpt:id` TOML blocks → `[artifacts.<KIND>.identifiers.<kind>]` entries with `[ref.<TARGET>]` sub-tables.
2. Aggregate all artifact constraints into one file keyed by `[artifacts.<KIND>]`.
3. Serialize as TOML with deterministic key ordering.

**Determinism guarantee**: same blueprints → identical `constraints.toml` (byte-for-byte).
...
Each `[[artifacts.<KIND>.headings]]` entry defines a constraint for one heading position in the artifact outline. Generated from `@cpt:heading` TOML blocks in the blueprint.
...
**Boolean convention** (aligned with blueprint): `true` = required, `false` = prohibited, omit = optional/allowed.

### ID Constraints

Each `[artifacts.<KIND>.identifiers.<kind>]` table defines validation rules for one ID kind. Generated from `@cpt:id` TOML blocks in the blueprint.
...
Each `[artifacts.<KIND>.identifiers.<kind>.ref.<TARGET>]` sub-table defines cross-artifact reference rules. Generated from `[ref.ARTIFACT]` sub-tables in `@cpt:id` TOML blocks.
